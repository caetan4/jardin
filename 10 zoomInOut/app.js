console.log("Árbol interactivo con Zoom");
...
const colores = ["#76dd15", "#f6cf5a", "#ff6b6b", "#6bafff"];
let index = 0;

botonColor.addEventListener("click", () => {
  index = (index + 1) % colores.length;
  copaMaterial.color.set(colores[index]);
  renderer.render(scene, camera);
});
...
zoomInBtn.addEventListener("click", () => {
  gsap.to(camera.position, { z: 2, duration: 1, ease: "power2.inOut", onUpdate: () => renderer.render(scene, camera) });
});

zoomOutBtn.addEventListener("click", () => {
  gsap.to(camera.position, { z: 5, duration: 1, ease: "power2.inOut", onUpdate: () => renderer.render(scene, camera) });
});
